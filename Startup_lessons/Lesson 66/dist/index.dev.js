"use strict";

var _express = _interopRequireDefault(require("express"));

var _mongoose = _interopRequireDefault(require("mongoose"));

var _user = require("./model/user.js");

var _jsonwebtoken = _interopRequireDefault(require("jsonwebtoken"));

var _cookieParser = _interopRequireDefault(require("cookie-parser"));

var _cors = _interopRequireDefault(require("cors"));

var _csurf = _interopRequireDefault(require("csurf"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

var checkToken = function checkToken(req, res, next) {
  var token = req.cookies.token;

  if (token) {
    _jsonwebtoken["default"].verify(token, secretKey, function (err, decoded) {
      if (err) {
        return res.status(401).json({
          message: 'Invalid token'
        });
      } else {
        next();
      }
    });
  } else {
    return res.status(401).json({
      message: 'Token not found'
    });
  }
};

var csrf = (0, _csurf["default"])({
  cookie: true
});
var PORT = 3000;
var secretKey = '123';
var url = 'mongodb://127.0.0.1:27017/study';
var app = (0, _express["default"])();
app.use((0, _cors["default"])({
  origin: 'https://itschool.com',
  optionsSuccessStatus: 200
}));
app.use(_express["default"]["static"]('css'));
app.use(_express["default"].urlencoded({
  extended: true
}));
app.use((0, _cookieParser["default"])());
app.set('view engine', 'pug');

_mongoose["default"].connect(url).then(function () {
  console.log('Connected to DB');
  app.listen(PORT, function () {
    console.log("Server started on http://localhost:".concat(PORT));
  });
})["catch"](function (err) {
  console.log("DB connection error: ".concat(err));
});

app.get('/', function _callee(req, res) {
  var title, isAdmin, users;
  return regeneratorRuntime.async(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          _context.prev = 0;
          title = {};
          isAdmin = false;

          if (req.cookies.username) {
            title.text = "You are in administrator mode, to logout press ";
            title.link = 'http://localhost:3000/logout';
            title.linkMessage = 'log out';
            isAdmin = true;
          } else {
            title.text = 'If you are admim please ';
            title.link = 'http://localhost:3000/login';
            title.linkMessage = 'log in';
            isAdmin = false;
          }

          _context.next = 6;
          return regeneratorRuntime.awrap(_user.User.find());

        case 6:
          users = _context.sent;
          res.render('index', {
            users: users,
            title: title,
            isAdmin: isAdmin
          });
          _context.next = 13;
          break;

        case 10:
          _context.prev = 10;
          _context.t0 = _context["catch"](0);
          console.log(_context.t0);

        case 13:
        case "end":
          return _context.stop();
      }
    }
  }, null, null, [[0, 10]]);
});
app.get('/user/:id', function _callee2(req, res) {
  var user;
  return regeneratorRuntime.async(function _callee2$(_context2) {
    while (1) {
      switch (_context2.prev = _context2.next) {
        case 0:
          _context2.prev = 0;
          _context2.next = 3;
          return regeneratorRuntime.awrap(_user.User.findById(req.params.id));

        case 3:
          user = _context2.sent;
          res.render('user', {
            user: user
          });
          _context2.next = 10;
          break;

        case 7:
          _context2.prev = 7;
          _context2.t0 = _context2["catch"](0);
          console.log(_context2.t0);

        case 10:
        case "end":
          return _context2.stop();
      }
    }
  }, null, null, [[0, 7]]);
});
app.post('/add', function _callee3(req, res) {
  var user;
  return regeneratorRuntime.async(function _callee3$(_context3) {
    while (1) {
      switch (_context3.prev = _context3.next) {
        case 0:
          _context3.prev = 0;
          user = new _user.User(req.body);
          _context3.next = 4;
          return regeneratorRuntime.awrap(user.save());

        case 4:
          res.redirect('/');
          _context3.next = 10;
          break;

        case 7:
          _context3.prev = 7;
          _context3.t0 = _context3["catch"](0);
          console.log(_context3.t0);

        case 10:
        case "end":
          return _context3.stop();
      }
    }
  }, null, null, [[0, 7]]);
});
app.get('/edit/:id', checkToken, function _callee4(req, res) {
  var user;
  return regeneratorRuntime.async(function _callee4$(_context4) {
    while (1) {
      switch (_context4.prev = _context4.next) {
        case 0:
          _context4.prev = 0;
          _context4.next = 3;
          return regeneratorRuntime.awrap(_user.User.findById(req.params.id));

        case 3:
          user = _context4.sent;
          res.render('edit', {
            user: user
          });
          _context4.next = 10;
          break;

        case 7:
          _context4.prev = 7;
          _context4.t0 = _context4["catch"](0);
          console.log(_context4.t0);

        case 10:
        case "end":
          return _context4.stop();
      }
    }
  }, null, null, [[0, 7]]);
});
app.post('/change-user/:id', function _callee5(req, res) {
  return regeneratorRuntime.async(function _callee5$(_context5) {
    while (1) {
      switch (_context5.prev = _context5.next) {
        case 0:
          _context5.prev = 0;
          _context5.next = 3;
          return regeneratorRuntime.awrap(_user.User.findByIdAndUpdate(req.params.id, req.body));

        case 3:
          res.redirect('/');
          _context5.next = 9;
          break;

        case 6:
          _context5.prev = 6;
          _context5.t0 = _context5["catch"](0);
          console.log(_context5.t0);

        case 9:
        case "end":
          return _context5.stop();
      }
    }
  }, null, null, [[0, 6]]);
});
app["delete"]('/remove/:id', function _callee6(req, res) {
  return regeneratorRuntime.async(function _callee6$(_context6) {
    while (1) {
      switch (_context6.prev = _context6.next) {
        case 0:
          _context6.prev = 0;
          _context6.next = 3;
          return regeneratorRuntime.awrap(_user.User.deleteOne({
            _id: req.params.id
          }));

        case 3:
          res.status(200).json({
            message: 'User deleted'
          });
          _context6.next = 9;
          break;

        case 6:
          _context6.prev = 6;
          _context6.t0 = _context6["catch"](0);
          console.log(_context6.t0);

        case 9:
        case "end":
          return _context6.stop();
      }
    }
  }, null, null, [[0, 6]]);
});
app.get('/login', csrf, function (req, res) {
  res.render('login', {
    csrfToken: req.csrfToken()
  });
});
app.post('/set-cookie', csrf, function _callee7(req, res) {
  var _req$body, name, password, token;

  return regeneratorRuntime.async(function _callee7$(_context7) {
    while (1) {
      switch (_context7.prev = _context7.next) {
        case 0:
          _req$body = req.body, name = _req$body.name, password = _req$body.password;

          if (name == 'admin' && password == '123') {
            token = _jsonwebtoken["default"].sign({
              name: name
            }, secretKey, {
              expiresIn: '1h'
            });
            res.cookie('token', token);
            res.cookie('username', name);
            res.redirect('/');
          } else {
            res.redirect('/login');
          }

        case 2:
        case "end":
          return _context7.stop();
      }
    }
  });
});
app.get('/logout', function (req, res) {
  res.clearCookie('username');
  res.clearCookie('token');
  res.redirect('/');
});
app.get('/api/users', function _callee8(req, res) {
  var users;
  return regeneratorRuntime.async(function _callee8$(_context8) {
    while (1) {
      switch (_context8.prev = _context8.next) {
        case 0:
          _context8.next = 2;
          return regeneratorRuntime.awrap(_user.User.find());

        case 2:
          users = _context8.sent;
          res.send({
            users: users
          });

        case 4:
        case "end":
          return _context8.stop();
      }
    }
  });
});